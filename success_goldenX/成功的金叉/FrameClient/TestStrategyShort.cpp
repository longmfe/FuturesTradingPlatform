...
void TestStrategyShort::run(){
	...
	int curBarIndex5=bar5->getLastBarNum(); //大周期bao0r index
	if(curBarIndex<11){  //当当前bar小于10+1时，不可能存在5周期均线和10周期均线死叉
		return;
	}
	...
	if(isFirst){  //当此tick为当前bar的第一个tick时，判断前一个bar是否开过死叉仓
	...
	if(cross && (diff>0.3 || diff<-0.3) && (!hasOpen)){
		...
		bar->hasOpenShortList.push_back(true);  //标记当前bar已经开过仓了
	}
	...
	if(bar5->getIsFirst()){
		...
		if(beforeLastDiff>0.0 && lastDiff<=0.0){  //大周期发生了死叉，判断其包含的小周期是否处于死叉状态
			int bigBarNum_cur=bar5->getLastBarNum();
			if((bigBarNum_cur-bigBarNum)==1 && smallSiCha){
				noClose=true;  //设置可平仓flag为false
			}
		}else if(beforeLastDiff<0.0 && lastDiff>=0.0){  //大周期发生了金叉，设置可平仓flag为true
			noClose=false;
		}
	}
