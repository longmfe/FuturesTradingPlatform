...
Order::Order(string strategyname, int id, string s, double price, Stype t, double slip, int vol, string date, string time,string exname
	, AccountManage* am)
	:type(Stype::Buy),
    //报单价格
    CommisionP(0.0),
...
Order::Order(string strategyname, int id, string s, double price, Stype t, double slip, int vol, string date, string time,string exname, AccountManage* am,double commsionrate,double marginrate,double multy)
	:type(Stype::Buy),
	//报单价格
	CommisionP(0.0),
	//保证金每单位
...
void Order::TradeOccur(double value, int vol){
		...
		toolsClass tc;
		str = "Strategy ID:" + StrategyName + split + "Sec:" + sec + split + "Direction:" + di + split + "Price:" +
			tc.doubleToString(value) + split
                  + "lots" + tc.doubleToString(vol) + split + "TradeTime:" + TradeAllTime + "," +
...
string Order::getTradeString()
{
	...
	toolsClass tc;
	str = "StrategyName" + StrategyName + split + "Sec:" + sec + split + "Direction:" + di + split + 
		
		"Price:" + tc.doubleToString(SendPrice) + split
		+ "lots" + tc.doubleToString(Volume) + split + "Date" + Date + split + "TradeTime:" + TradeAllTime;
	return str;
}
